...
> = ({ actions, children, className, hideCloseButton = false, showOverlay = true, ...props }) => (
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      className={cn(
        `
          fixed top-1/2 left-1/2 z-50 grid max-h-[95vh] w-full max-w-[95vw] -translate-x-1/2 -translate-y-1/2 gap-4
          overflow-y-auto border bg-background p-4 shadow-lg duration-200
...
const DialogHeader: React.FC<React.ComponentProps<'div'>> = ({ className, ...props }) => (
  <div
    className={cn(
      `
        mb-2 flex flex-col space-y-2.5 text-center
        sm:text-left
      `,
      className,
    )}
